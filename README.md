...
RapidAdult = data.frame(subset(RapidAdult, gpraAdultAll.YEAR.x == 2016))
...
E_NONHISPAN = data.frame(gpraAdultAll$E_NONHISPAN)
hispanic = data.frame(E_NONHISPAN, gpraAdultAll$E_MEXICAN, gpraAdultAll$E_PUERTRICAN, gpraAdultAll$E_CUBAN, gpraAdultAll$E_OTHERHISPAN)
...
hispanic$Percentage = hispanic$Count /n
write.csv(hispanic, "hispanic.csv", row.names = FALSE)
...
asian$Percentage = asian$Count /n
...
black$Percentage = black$Count /n
...
edu = count(edu)
rownames(edu) = c("High School", "Community College", "Four-year college", "Beyond four-year college", "NA", "NA1")
...
### APHA and Paper analyses ######
## Need the count for yesses and nos for each group then do McNemar test
# First up is unprotected sex
# Need n as well.

Goal3ObjectiveMBaseMonth3 = data.frame(gpraAdultAll$LASTSEX_UNP.x, gpraAdultAll$LASTSEX_UNP.y)
Goal3ObjectiveMBaseMonth3 = data.frame(apply(Goal3ObjectiveMBaseMonth3, 2, function(x){ifelse(x == 98, NA, ifelse(x == 99, NA, x))}))
Goal3ObjectiveMBaseMonth3 = data.frame(na.omit(Goal3ObjectiveMBaseMonth3))
# Now a 1 means you are having unprotected sex instead of protected.  So a 0 means no you are not having unprotected sex and a 1 means you are having unprotected sex.
Goal3ObjectiveMBaseMonth3 = data.frame(apply(Goal3ObjectiveMBaseMonth3, 2, function(x){ifelse(x == 1, 0, 1)}))
colnames(Goal3ObjectiveMBaseMonth3) = c("Base", "Month3")
dim(Goal3ObjectiveMBaseMonth3)

UnproBase = count(Goal3ObjectiveMBaseMonth3$Base)
UnproBaseNo = UnproBase[1,2]
UnproBaseYes = UnproBase[2,2]

UnproMonth3 = count(Goal3ObjectiveMBaseMonth3$Month3)
UnproMonth3No = UnproMonth3[1,2]
UnproMonth3Yes = UnproMonth3[2,2]

UnproDat = matrix(c(UnproBaseYes, UnproBaseNo, UnproMonth3Yes, UnproMonth3No), nrow = 2, byrow = TRUE)
# I need to know the number of times all answers were yes and no and the number of times they disagreed.
# So an ifelse stating that if your response is 1 on both then you get a one 
UnproDat
